...
userSchema.statics.processProfile = profile => {
  ...
  if (_.isArray(profile.emails)) {
    ...
    profile.user &&
    profile.user.email &&
    profile.user.email.length > 5
  ) {
    primaryEmail = profile.user.email;
  } else if (
    _.isString(profile.unique_name) &&
    profile.unique_name.length > 5
  ) {
  ...
  return db.User.findOneAndUpdate(
    ...
    if (
      ...
      (appconfig.auth.defaultReadAccess ||
    ...
    return (
      user || Promise.reject(new Error(lang.t('auth:errors:notyetauthorized')))
    );
  });
};
